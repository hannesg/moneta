...
describe_moneta "simple_tokyocabinet_with_expires" do
  def log
    @log ||= File.open(File.join(make_tempdir, 'simple_tokyocabinet_with_expires.log'), 'a')
  end

  def new_store
    Moneta.new(:TokyoCabinet, :file => File.join(make_tempdir, "simple_tokyocabinet_with_expires"), :expires => true, :logger => {:out => log})
  end
